...
function catchLocalMediaStream(err){
    console.log(err)
}
...
function remoteAction(){
    ...
    localPeerConnection.addEventListener('iceconnectionstatechange', handleConnectionChanged); // 변경 감지 이벤트 핸들러
    ...
    remotePeerConnection.addEventListener('iceconnectionstatechange', handleConnectionChanged); // 변경 감지 이벤트 핸들러
...
function createdOffer(des){
    ...
    // 로컬 피어가 offer 정보를 내가 전송하고자 하는 정보로 섫정하게 된다.
    localPeerConnection.setLocalDescription(des)
    .then(() => console.log('connection established in localPeer connection'))
    .catch((err) => console.log(`connection error: ${err}`));

    remotePeerConnection.setRemoteDescription(des)
    .then(() => console.log('connection established in remote connection'))
    .catch((err) => console.log(`connection in remote peerConnection error: ${error}`));

    remotePeerConnection.createAnswer()
    .then((answer)=>{
        remotePeerConnection.setLocalDescription(answer)
        .then(()=> console.log('success to send answer from remote peer connection'))
        .catch((error)=> console.err('error to send answer from remote peer connection: ', error));
    
        localPeerConnection.setRemoteDescription(answer)
        .then(() => console.log('success to set answer from local peer connection'))
        .catch(() => console.err('error to set answer local peer connection'));
    })
    .catch((err)=> console.err('failed to answer to local peer connection'));
} 

function catchOfferCreated(error){
    console.log(`error to create offer: ${error}`);
}
...
function handleConnection(event){
    ...
    if( ice) { 
        ...
        .then(() => console.log('added ice candidate success'))
        .catch((err) => console.log('error adding ice candidate success',err));
